...
@pytest.fixture    
def y_true_vert():
    return tf.constant([[1.0, 0.0], [1.0, 0.0], [0.0, 1.0]])

@pytest.fixture
def y_pred_vert():
    return tf.constant([[1.0, 0.0], [0.0, 1.0], [0.5, 0.5]])

@pytest.fixture
def y_true_horiz(): 
    return tf.transpose(tf.constant([[1.0, 0.0], [1.0, 0.0], [0.0, 1.0]]))

@pytest.fixture
def y_pred_horiz():
    return tf.transpose(tf.constant([[1.0, 0.0], [0.0, 1.0], [0.5, 0.5]]))
...
def test_corner_preds_vert_macro(y_true_vert, y_pred_vert):
    assert pytest.approx(21/35) == f1_loss(y_true_vert, y_pred_vert, 'macro')
...
def test_corner_preds_horiz_macro(y_true_horiz, y_pred_horiz):
    assert pytest.approx(0.5) == f1_loss(y_true_horiz, y_pred_horiz, 'macro')
